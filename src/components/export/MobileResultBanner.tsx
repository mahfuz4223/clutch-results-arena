        ...
        <table className="w-full border-collapse bg-black/80 text-white">
          ...
          <tbody>
            {standings.slice(0, 8).map((standing, index) => (
              ...
              >
                <td className="py-1 px-1 font-bold">{index + 1}</td>
                <td className="py-1 px-1">
                  {standing.teamFlag && <span className="mr-1">{standing.teamFlag}</span>}
                  <span className="font-medium">{standing.teamName}</span>
                </td>
                <td className="py-1 px-1 text-center">{standing.wwcd}</td>
                <td className="py-1 px-1 text-center">{standing.totalPlacementPoints}</td>
                <td className="py-1 px-1 text-center">{standing.totalKills}</td>
                <td className="py-1 px-1 text-center font-bold">{standing.totalPoints}</td>
              </tr>
            ))}
          </tbody>
        </table>

        {/* Second half of standings */}
        <table className="w-full border-collapse bg-black/80 text-white mt-2">
          ...
          <tbody>
            {standings.slice(8, 16).map((standing, index) => (
              ...
              >
                <td className="py-1 px-1 font-bold">{index + 9}</td>
                <td className="py-1 px-1">
                  {standing.teamFlag && <span className="mr-1">{standing.teamFlag}</span>}
                  <span className="font-medium">{standing.teamName}</span>
                </td>
